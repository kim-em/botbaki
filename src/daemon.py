...
"""
    ...
    def _handle_feedback(self, trigger: dict, args: str) -> None:
        ...
        self.log(f"  Storing feedback ({len(feedback_text)} chars)")
        ...
        db.execute("""
            ...
            (review_id, pr_id, pr_number, comment_id, feedback_text, github_login, created_at, processed_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            review_id,
            pr_id,
        ...
        # Post acknowledgment
        if review_id:
            response = f"@{author} Thanks for the feedback! I've recorded it for review #{review_id}."
        else:
            response = f"@{author} Thanks for the feedback! I've recorded it (no review found to link to)."
        ...
        try:
            ...
            self._mark_processed(
                comment_id, pr_id, author, "feedback",
                response_comment_id=comment_result.get('id')
            )
            self.log(f"  Posted ack: {comment_result.get('html_url')}")
        except GitHubError as e:
            # Still mark as processed even if ack fails
            self._mark_processed(comment_id, pr_id, author, "feedback", error=str(e))
            self.log(f"  Stored feedback but failed to post ack: {e}")
